...
for a in np.arange(410):
    ...
    try:
        ...
        for aa in np.arange(len(drifters['time'])):
            ...
            dd.append(loc_dt.strftime(fmt)[0:19])
    ...
    try:
        day=(datetime.strptime(d['time'][-1], '%Y-%m-%d'+' '+'%H:%M:%S')-datetime.strptime(d['time'][0], '%Y-%m-%d'+' '+'%H:%M:%S')).days
    except:
        continue
    ...
    aa=a
    d_time=datetime.strptime(d['time'][0], '%Y-%m-%d'+' '+'%H:%M:%S')       
    start_times=d_time
    rawtime=[]
    for a in np.arange(len(d['time'])):
        rawtime.append(date2num(datetime.strptime(d['time'][a], '%Y-%m-%d'+' '+'%H:%M:%S')))
