...
class get_fvcom():
    ...
    def get_url(self, starttime, endtime):
        ...
        if self.modelname == "GOM3":
            ...
            #print Times
            #print len(Times[0])
            c=self.Times2[index1]
            d=datetime.strptime(c[:-7], '%Y-%m-%d'+'T'+'%H:%M:%S')
            print 'Times[index1]',type(d)
            print 'Times[index1]',d
            for a in xrange(t):
                if (d+timedelta(hours=a)).month!=(d).month:
                    self.m=a
    ...
    def get_track(self,lon,lat,depth,starttime,wind,wind_get_type): #,b_index,nvdepth, 
        ...
        for i in xrange(t):  
            ...
            if i!=0 and i%24==0 :
                ...
                #lonl,latl = self.shrink_data(lon,lat,self.lonc,self.latc,0.8)
            ...
            #print self.u[i,layer]
            print 'u_t,v_t',u_t,v_t#u_t1 = self.u[i,layer,elementindex][0]; v_t1 = self.v[i,layer,elementindex][0]
